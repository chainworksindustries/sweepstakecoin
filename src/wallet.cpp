    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s SWEEP %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                {
                    printf("ERROR: ReacceptWalletTransactions() : txindex.vSpent.size() %lu != wtx.vout.size() %lu\n", txindex.vSpent.size(), wtx.vout.size());
                    continue;
                }
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s SWEEP %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
// SweepstakeCoin: total coins staked (non-spendable until maturity)
...
{
    {
        ...
        if (block.IsProofOfWork() && mapWallet.count(block.vtx[0].GetHash()))
        {
            ...
            printf("    mine:  %d  %d  %u", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
        }
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            CWalletTx& wtx = mapWallet[block.vtx[1].GetHash()];
            printf("    stake: %d  %d  %u", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
         }
...
{
    {
        ...
        printf("CWallet::NewKeyPool wrote %u new keys\n", nKeys);
    }
    return true;
}
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            printf("keypool added key %u, size=%lu\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
...
// SweepstakeCoin: check 'spent' consistency between wallet and txindex
// SweepstakeCoin: fix wallet spent state according to txindex
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s SWEEP %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s SWEEP %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
// SweepstakeCoin: disable transaction (only for coinstake)
