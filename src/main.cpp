...
unsigned int nWorkTargetSpacing = 60;                   // 60 sec block spacing for PoW
...
const string strMessageMagic = "SweepstakeCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // SweepstakeCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // SweepstakeCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
    ...
    if (fCheckInputs)
    {
        ...
        if (nFees < txMinFee)
            return error("CTxMemPool::accept() : not enough fees %s, %u < %lu",
                         hash.ToString().c_str(),
                         nFees, txMinFee);
...
const int POW_BLOCKS_PER_DAY = 1440;
const double SWEEPSTAKE_MAX_ACCUMULATE_DAY = 10.0;
const int MAX_RANDOM_RANGE = 720000;
const double SWEEPSTAKE_EXP_CONSTANT = 0.5991464547;
...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	...
	int random = GenerateMTRandom(seed, 720000);

	double numofdays = (double)delta / (double)POW_BLOCKS_PER_DAY;
	if (numofdays > SWEEPSTAKE_MAX_ACCUMULATE_DAY)
	{
		// increase the possibility
		int dd = 5 * (delta - SWEEPSTAKE_MAX_ACCUMULATE_DAY * POW_BLOCKS_PER_DAY);
		if (dd < 0)
			dd = 0;
		...
		upperLimit += dd;
		if (upperLimit > MAX_RANDOM_RANGE)
	...
	if (random > lowerLimit && random < upperLimit)	// 1 in 5 days on average
	{
		...
		double dfac = 50.0 * exp(SWEEPSTAKE_EXP_CONSTANT * numofdays) * 100.0;
		return (int64_t)(dfac - 100);
	}
...
int64_t GetCurrentSweepstakeSize(const CBlockIndex* pindex)
{
	...
	double dfac = 50.0 * exp(SWEEPSTAKE_EXP_CONSTANT * numofdays);
	return (int64_t)((double)reward * dfac) / COIN;
}
...
// SweepstakeCoin: find last block index up to pindex
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound: invalid block=%s  height=%d  trust=%s  blocktrust=%u  date=%s\n",
      pindexNew->GetBlockHash().ToString().c_str(), pindexNew->nHeight,
      CBigNum(pindexNew->nChainTrust).ToString().c_str(), nBestInvalidBlockTrust.Get64(),
      DateTimeStrFormat("%x %H:%M:%S", pindexNew->GetBlockTime()).c_str());
    printf("InvalidChainFound:  current best=%s  height=%d  trust=%s  blocktrust=%u  date=%s\n",
                               ...
                               bool fBlock, bool fMiner, MapPrevTx& inputsRet, bool& fInvalid)
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        if (prevout.n >= txPrev.vout.size() || prevout.n >= txindex.vSpent.size())
        {
            ...
            // adding inputs:
            fInvalid = true;
            return DoS(100, error("FetchInputs() : %s prevout.n out of range %d %lu %lu prev tx %s\n%s", GetHash().ToString().substr(0,10).c_str(), prevout.n, txPrev.vout.size(), txindex.vSpent.size(), prevout.hash.ToString().substr(0,10).c_str(), txPrev.ToString().c_str()));
        }
    }
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // SweepstakeCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SweepstakeCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        ...
        if (vtx[0].GetValueOut() > nReward)
            return DoS(50, error("ConnectBlock() : coinbase reward exceeded (actual=%u vs calculated=%u)",
                   vtx[0].GetValueOut(),
                   nReward));
    }
    ...
    if (IsProofOfStake())
    {
        // SweepstakeCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
        ...
        if (nStakeReward > nCalculatedStakeReward)
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%u vs calculated=%u)", nStakeReward, nCalculatedStakeReward));
    }

    // SweepstakeCoin: track money supply and mint amount info
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    else
    {
        ...
        if (!vpindexSecondary.empty())
            printf("Postponing %lu reconnects\n", vpindexSecondary.size());
    ...
    printf("SetBestChain: new best=%s  height=%d  trust=%s  blocktrust=%u  date=%s\n",
      hashBestChain.ToString().c_str(), nBestHeight,
...
// SweepstakeCoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (fDebug && GetBoolArg("-printcoinage"))
            printf("coin age nValueIn=%u nTimeDiff=%d bnCentSecond=%s\n", nValueIn, nTime - txPrev.nTime, bnCentSecond.ToString().c_str());
    }
...
// SweepstakeCoin: total coin age spent in block, in the unit of coin-days.
bool CBlock::GetCoinAge(uint64_t& nCoinAge) const
{
    ...
    if (nCoinAge == 0) // block coin age minimum 1 coin-day
        nCoinAge = 1;
    if (fDebug && GetBoolArg("-printcoinage"))
        printf("block coin age total nCoinDays=%u\n", nCoinAge);
    return true;
}
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // SweepstakeCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // SweepstakeCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // SweepstakeCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // SweepstakeCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
	...
	int nPowHeight = GetPowHeight(pindexPrev);
	...
	for (unsigned i = 1; i < vtx.size(); i++)
		if (vtx[i].IsCoinBase())
		{
			...
			for (unsigned j = 0; j < vtx[i].vout.size(); j++)
				if (prevBlock.vtx[0].vout[j].nValue * nBonusReward / 100 != vtx[i].vout[j].nValue ||
    ...
    // SweepstakeCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // SweepstakeCoin: check proof-of-stake
    ...
    // SweepstakeCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // SweepstakeCoin: getblocks may not obtain the ancestor block rejected
    ...
    // SweepstakeCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SweepstakeCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
		...
		const char* pszTimestamp = "SweepstakeCoin: your chance is there!!!";
        CTransaction txNew;
        txNew.nTime = 1493957506;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1493957509;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 7209625;
		block.nSuperBlock = 0;
        ...
        //// debug print
		assert(block.hashMerkleRoot == uint256("0x7b43fbdd9e7b664deb905aa749fb2e611b308ca71060102470f8370d68d514f1"));
		block.print();
        ...
        // SweepstakeCoin: initialize synchronized checkpoint
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  mint %7s  tx %lu",
            pindex->nHeight,
            pindex->nFile,
            pindex->nBlockPos,
            block.GetHash().ToString().c_str(),
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    printf("Loaded %i blocks from external file in %ums\n", nLoaded, GetTimeMillis() - nStart);
    return nLoaded > 0;
}
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc1, 0xfc, 0xdb };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // SweepstakeCoin: send latest proof-of-work block to allow the
