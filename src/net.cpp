...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-ext-ip");
...
void ThreadSocketHandler(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-net");
            ...
            if (FD_ISSET(pnode->hSocket, &fdsetRecv) || FD_ISSET(pnode->hSocket, &fdsetError))
            {
                ...
                if (lockRecv)
                {
                    ...
                    if (nPos > ReceiveBufferSize()) {
                        if (!pnode->fDisconnect)
                            printf("socket recv flood control disconnect (%lu bytes)\n", vRecv.size());
                        pnode->CloseSocketDisconnect();
                    }
                    else {
...
void ThreadMapPort(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-UPnP");
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SweepstakeCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void DumpAddresses()
{
    ...
    printf("Flushed %d addresses to peers.dat  %lums\n",
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-adrdump");
...
{
    // Make this thread recognisable as the connection opening thread
    RenameThread("SweepstakeCoin-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SweepstakeCoin is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-start");
