// Copyright (c) 2014 The SweepstakeCoin developers
...
void SecMsgBucket::hashBucket()
{
    ...
    if (fDebugSmsg)
        printf("Hashed %lu messages, hash %u\n", setTokens.size(), hash);
};
...
void ThreadSecureMsg(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-smsg"); // Make this thread recognisable
    ...
    while (fSecMsgEnabled)
    {
        ...
        
        {
            ...
            {
                //if (fDebugSmsg)
                //    printf("Checking bucket %u, size %lu \n", it->first, it->second.setTokens.size());
                if (it->first < cutoffTime)
                {
                    if (fDebugSmsg)
                        printf("Removing bucket %u \n", it->first);
                ...
                } else
                {
                    ...
                    if (it->second.nLockCount > 0)
                    {
                        ...
                        if (it->second.nLockCount == 0)     // lock timed out
                        {
                            ...
                            if (fDebugSmsg)
                                printf("Lock on bucket %u for peer %u timed out.\n", it->first, nPeerId);
                            ...
                            BOOST_FOREACH(CNode* pnode, vNodes)
                            {
                                ...
                                if (fDebugSmsg)
                                    printf("This node will ignore peer %u until %u.\n", nPeerId, ignoreUntil);
                                break;
                            };
...
void ThreadSecureMsgPow(void* parg)
{
    ...
    RenameThread("SweepstakeCoin-smsg-pow"); // Make this thread recognisable
...
int SecureMsgBuildBucketSet()
{
    ...
    */
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        if (fDebugSmsg)
            printf("Bucket %u contains %lu messages.\n", fileTime, tokenSet.size());
    };
    
    printf("Processed %u files, loaded %lu buckets containing %u messages.\n", nFiles, smsgBuckets.size(), nMessages);
    ...
    */
    ...
    if (strCommand == "smsgInv")
    {
        ...
        for (uint32_t i = 0; i < nInvBuckets; ++i)
        {
            ...
            {
                if (fDebugSmsg)
                    printf("Not interested in peer bucket %u, has expired.\n", time);
            ...
            {
                if (fDebugSmsg)
                    printf("Not interested in peer bucket %u, in the future.\n", time);
                pfrom->Misbehaving(1);
                continue;
            };
            ...
            {
                if (fDebugSmsg)
                    printf("Peer sent empty bucket, ignore %u %u %u.\n", time, ncontent, hash);
                continue;
            };
            
            if (fDebugSmsg)
            {
                printf("peer bucket %u %u %u.\n", time, ncontent, hash);
                printf("this bucket %u %lu %u.\n", time, smsgBuckets[time].setTokens.size(), smsgBuckets[time].hash);
            };
            ...
            {
                if (fDebugSmsg)
                    printf("Requesting contents of bucket %u.\n", time);
    ...
    if (strCommand == "smsgShow")
    {
        ...
        for (uint32_t i = 0; i < nBuckets; ++i, pIn += 8)
        {
            ...
            try {
                ...
                printf("vchDataOut.resize %lu threw: %s.\n", 8 + 16 * tokenSet.size(), e.what());
                continue;
            };
    ...
    if (strCommand == "smsgHave")
    {
        ...
        {
            if (fDebugSmsg)
                printf("Bucket %u lock count %u, waiting for message data from peer %u.\n", time, smsgBuckets[time].nLockCount, smsgBuckets[time].nLockPeerId);
            return false;
        }; 
        
        if (fDebugSmsg)
            printf("Sifting through bucket %u.\n", time);
        ...
        if (vchDataOut.size() > 8)
        {
            if (fDebugSmsg)
            {
                printf("Asking peer for  %lu messages.\n", (vchDataOut.size() - 8) / 16);
                printf("Locking bucket %lu for peer %u.\n", time, pfrom->smsgData.nPeerId);
            };
    ...
    if (strCommand == "smsgWant")
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            {
                if (fDebugSmsg)
                    printf("Don't have wanted message %u.\n", token.timestamp);
            } else
            {
                //printf("Have message at %u.\n", it->offset); // DEBUG
                token.offset = it->offset;
                //printf("winb before SecureMsgRetrieve %u.\n", token.timestamp);
                ...
                {
                    printf("SecureMsgRetrieve failed %u.\n", token.timestamp);
                };
        ...
        {
            if (fDebugSmsg)
                printf("Sending block of %u messages for bucket %u.\n", nBunch, time);
    ...
    if (strCommand == "smsgMatch")
    {
        ...
        if (fDebugSmsg)
            printf("Peer buckets matched at %u.\n", time);
    ...
    if (strCommand == "smsgIgnore")
    {
        ...
        if (vchData.size() < 8)
        {
            printf("smsgIgnore, not enough data %lu.\n", vchData.size());
            pfrom->Misbehaving(1);
            return false;
        };
        ...
        if (fDebugSmsg)
            printf("Peer %u is ignoring this node until %u, ignore peer too.\n", pfrom->smsgData.nPeerId, time);
    } else
    {
        // Unknown message
    };
        ...
        if (nBuckets > 0) // no need to send keep alive pkts, coin messages already do that
        {
            ...
            for (it = smsgBuckets.begin(); it != smsgBuckets.end(); ++it)
            {
                ...
                try {
                    ...
                    printf("vchData.resize %lu threw: %s.\n", vchData.size() + 16, e.what());
                    continue;
                };
                memcpy(p, &it->first, 8);
                ...
                nBucketsShown++;
                //if (fDebug)
                //    printf("Sending bucket %u, size %d \n", it->first, it->second.size());
            };
...
{
    if (fDebugSmsg)
        printf("SecureMsgRetrieve() %u.\n", token.timestamp);
    
    // -- has cs_smsg lock from SecureMsgReceiveData
    
    fs::path pathSmsgDir = GetDataDir() / "smsgStore";
    
    //printf("token.offset %u.\n", token.offset); // DEBUG
...
int SecureMsgReceive(CNode* pfrom, std::vector<unsigned char>& vchData)
{
    ...
    {
        printf("Error: Invalid no. messages received in bunch %u, for bucket %u.\n", nBunch, bktTime);
        pfrom->Misbehaving(1);
...
int SecureMsgStore(unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload, bool fUpdateBucket)
{
    ...
    
    {
        ...
        if (it != tokenSet.end())
        {
            ...
            if (fDebugSmsg)
            {
                ...
                printf("message ts: %lu", token.timestamp);
                ...
                for (it = tokenSet.begin(); it != tokenSet.end(); ++it)
                {
                    printf("message ts: %lu", (*it).timestamp);
                    vchShow.resize(8);
    ...
    //if (fDebugSmsg)
    printf("SecureMsg added to bucket %u.\n", bucket);
    return 0;
};
...
int SecureMsgSetHash(unsigned char *pHeader, unsigned char *pPayload, uint32_t nPayload)
{
    ...
    */
    ...
    {
        if (fDebugSmsg)
            printf("SecureMsgSetHash() failed, took %u ms, nonse %u\n", GetTimeMillis() - nStart, nonse);
        return 1;
    };
    ...
    if (fDebugSmsg)
        printf("SecureMsgSetHash() took %u ms, nonse %u\n", GetTimeMillis() - nStart, nonse);
